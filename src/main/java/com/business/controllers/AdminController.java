...
import com.business.loginCredentials.CustomerLogin;
...
public class AdminController {
	...
	public String userLogin( @ModelAttribute("userLogin") CustomerLogin login,Model model)
	{
		...
		if(services.validateLoginCredentials(email, password))
		{
			...
			List<Purchase> orders = this.orderServices.getOrdersForUser(user);
	...
	public String seachHandler(@RequestParam("productName") String name,Model model)
	{
		...
		Item product=this.productServices.getProductByName(name);
		if(product==null)
		{
	...
	{
		List<Customer> users= this.services.getAllUser();
		List<Admin>admins=this.adminServices.getAll(); 
		List<Item>products=this.productServices.getAllProducts();
		List<Purchase> orders = this.orderServices.getOrders();
	...
	{
		Item product=this.productServices.getProduct(id);
	...
	{
		Customer user = this.services.getUser(id);
		model.addAttribute("user", user);
		return "Update_User";
	}
	//Placing  Order
	@PostMapping("/product/order")
	public String orderHandler(@ModelAttribute() Purchase order,Model model)
	{
